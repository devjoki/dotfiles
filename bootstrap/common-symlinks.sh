...
# Link oh-my-posh config
if [ -f "$SCRIPT_DIR/ohmyposh.toml" ] && [ "$SCRIPT_DIR/ohmyposh.toml" != "$HOME/.config/ohmyposh.toml" ]; then
	create_symlink "$SCRIPT_DIR/ohmyposh.toml" "$HOME/.config/ohmyposh.toml" --override
fi

# Link Oh My Posh themes
if [ -d "$SCRIPT_DIR/omp_themes" ] && [ "$SCRIPT_DIR/omp_themes" != "$HOME/.config/omp_themes" ]; then
	create_symlink "$SCRIPT_DIR/omp_themes" "$HOME/.config/omp_themes" --override
fi
